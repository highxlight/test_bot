...
class PrivateSocekt{
    ...
    async getUserID(){
        ...
        try{
            logger.debug(`aax.future.socket.private.getUserid.url: ${endpoint}`);
            await this.axiosInstance.get(endpoint,{
                ...
                let dicData = {
                    code:400,
                    msg: 'aax.future.socket.private.getUserid:unknown error'
                }
                if(err.response){
                ...
                logger.error(`aax.future.socket.private.getUserid.err: ${err}`);
            })
            logger.info(`aax.future.socket.private.getUserid.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`aax.future.socket.private.getUserid.error: ${error}`);
            return response;
        }
    }
            ...
            try{
                ...
                ws.on('close', (closeEventCode, reason) => {
                    if (!wsRef.closeInitiated) {
                        logger.error(`aax future privateSocket connection close due to ${closeEventCode}: ${reason}.`)
                        setTimeout(() => {
                            logger.debug('aax future privateSocket reconnect to the server.')
                            initConnect()
                        }, this.reconnectDelay)
                    } else {
                        wsRef.closeInitiated = false
                ...
                logger.error(`aax.future.private.socket.error:${err.message}`)
            }
    ...
    unsubscribe (objwsRef) {
        if (!objwsRef || !objwsRef.ws) {
            logger.warn('aax future privateSocket connection to close.')
        }else {
            try {
                objwsRef.unMsg.op = "#unsubscribe";
                objwsRef.ws.send(JSON.stringify(objwsRef.unMsg));
            }catch (err){
                logger.error(`aax future privateSocket connection to close.err: ${err.message}`)
            }
