...
class Account{
    ...
    async getAccountBalance(){
        ...
        const endpoint='/v2/account/balances?purseType=FUTP';
        ...
        try{
            logger.debug(`aax.future.account.getAccountBalance.url: ${endpoint}`);
            await this.axiosInstance.get(endpoint,{
                ...
                let dicData = {
                    code:400,
                    msg: 'aax.future.account.getAccountBalance:unknown error'
                }
                if(err.response){
                ...
                logger.error(`aax.future.account.getAccountBalance.err: ${err}`);
            })
            logger.info(`aax.future.account.getAccountBalance.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`aax.future.account.getAccountBalance.error: ${error}`);
            return response;
        }
    }
}
