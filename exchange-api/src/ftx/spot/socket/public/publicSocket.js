...
class PublicSocket{
    ...
    dataFormat(payload){
        ...
        switch (e_type){
            case "orderbook":
                ...
                this.symbolOrderBookEE.emit('FTX_OB_UPDATES', orderBookData);
                break;
        }
    }
        ...
        const initConnect = () => {
            ...
            ws.on('open', ()=>{
                wsRef.timer = setInterval(()=>{
                    try{
                        ...
                        console.log(`ftx.spot.socket.public.socket.timer.err: ${err.message}`);
                    }
                }, 15000)
            ...
            ws.on('close', (closeEventCode, reason) => {
                if (!wsRef.closeInitiated) {
                    logger.error(`ftx spot publicSocket connection close due to ${closeEventCode}: ${reason}.`)
                    setTimeout(() => {
                        logger.debug('ftx spot publicSocket reconnect to the server.')
                        initConnect()
                    }, this.reconnectDelay)
                } else {
                    wsRef.closeInitiated = false
    ...
    unsubscribe (objRef) {
        if (!objRef || !objRef.ws) logger.warn('ftx spot publicSocket no connection to close.')
        else {
            try {
                ...
                logger.error(`ftx spot publicSocket no connection to close.err: ${err.message}`)
            }
