...
class PrivateSocekt{
        ...
        switch (e_type){
            case 'orders':
                ...
                let updata = {
                    ...
                    quoteQty: parseFloat(pl.data.filledSize) * parseFloat(pl.data.avgFillPrice),
                    updateTime: '',
                    rawData: pl
                }
        ...
        const initConnect = () => {
            ...
            ws.on('close', (closeEventCode, reason) => {
                if (!wsRef.closeInitiated) {
                    logger.error(`ftx spot privateSocket connection close due to ${closeEventCode}: ${reason}.`)
                    setTimeout(() => {
                        logger.debug('ftx spot privateSocket reconnect to the server.')
                        initConnect()
                    }, this.reconnectDelay)
                } else {
                    wsRef.closeInitiated = false
    ...
    unsubscribe (objRef) {
        if (!objRef || !objRef.ws) logger.warn('ftx spot privateSocket no connection to close.')
        else {
            try {
                objRef.unMsg.op = 'unsubscribe';
                objRef.ws.send(JSON.stringify(objRef.unMsg));
            }catch (err){
                logger.error(`ftx spot privateSocket no connection to close.err: ${err.message}`)
            }
