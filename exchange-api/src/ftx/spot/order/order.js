...
class Order{
    ...
    async createOrder(fsym, tsym, side, type, options){
        ...
        try{
            logger.debug(`ftx.spot.order.createOrder.url: ${endpoint},  params: ${JSON.stringify(requestBody)}`);
            await this.axiosInstance.post(endpoint, requestBody,{
                ...
                let dicData = {
                    code:400,
                    msg: 'ftx.spot.order.createOrder:unknown error'
                }
                if(err.response){
                ...
                logger.error(`ftx.spot.order.createOrder.err: ${err}`);
            })
            logger.info(`ftx.spot.order.createOrder.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`ftx.spot.order.createOrder.error: ${error}`);
            return response;
        }
    }
    ...
    async cancelSingle (fsym, tsym, orderId){
        ...
        try {
            logger.debug(`ftx.spot.order.cancelSingle.url: ${endpoint}`);
            await this.axiosInstance.delete(endpoint,{
                ...
                let dicData = {
                    code:400,
                    msg: 'ftx.spot.order.cancelSingle:unknown error'
                }
                if(err.response){
                    dicData.code = err.response.data.success;
                    dicData.msg = err.response.data.error;
                ...
                response.data = dicData;
                logger.error(`ftx.spot.order.cancelSingle.err: ${err}`);
            })
            logger.info(`ftx.spot.order.cancelSingle.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`ftx.spot.order.cancelSingle.error: ${error}`);
            return response;
        }
    }
    ...
    async getOrder(fsym, tsym, orderId){
        ...
        try {
            ...
            await this.axiosInstance.get(endpoint,{
                ...
                let dicData = {
                    code:400,
                    msg: 'ftx.spot.order.getOrder:unknown error'
                }
                if(err.response){
                    dicData.code = err.response.data.success;
                    dicData.msg = err.response.data.error;
                ...
                response.data = dicData;
                logger.error(`ftx.spot.order.getOrder.err: ${err}`);
            })
            logger.info(`ftx.spot.order.getOrder.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`ftx.spot.order.getOrder.error: ${error}`);
            return response;
        }
    }
    ...
    async getOrders(fsym, tsym, options){
        ...
        try {
            logger.debug(`ftx.spot.order.getOrders.url: ${endpoint}`);
            await this.axiosInstance.get(endpoint,{
                headers:this.#header
            }).then(res=>{
                if(res.data.success){
                ...
                let dicData = {
                    code:400,
                    msg: 'ftx.spot.order.getOrders:unknown error'
                }
                if(err.response){
                    dicData.code = err.response.data.success;
                    dicData.msg = err.response.data.error;
                ...
                response.data = dicData;
                logger.error(`ftx.spot.order.getOrders.err: ${err}`);
            })
            logger.info(`ftx.spot.order.getOrders.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`ftx.spot.order.getOrders.error: ${error}`);
            return response;
        }
    }
}
