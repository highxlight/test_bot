...
class PrivateSocekt{
    ...
    loign(){
        ...
        let presigned = `access_id=${this.#apiKey}&timestamp=${timestamp}&secret_key=${this.#secretKey}`;
        let sign =  createHash('sha256').update(presigned).digest('hex')
        let msg ={
            "method": "server.sign",
            "params": [
                this.#apiKey,
        ...
        const initConnect = () => {
            try{
                ...
                ws.on('message', (data)=>{
                    ...
                    if(pl.error !== null){
                        logger.error(`coinex.future.socket.private.error: ${JSON.stringify(pl)}`);
                ...
                ws.on('close', (closeEventCode, reason) => {
                    if (!wsRef.closeInitiated) {
                        logger.error(`coinex future privateSocket connection close due to ${closeEventCode}: ${reason}.`)
                        setTimeout(() => {
                            logger.debug('coinex future privateSocket reconnect to the server.')
                            initConnect()
                        }, this.reconnectDelay)
                    } else {
                        wsRef.closeInitiated = false
    ...
    unsubscribe (objwsRef) {
        if (!objwsRef || !objwsRef.ws) {
            logger.warn('coinex future privateSocket no connection to close.')
        }else {
            try {
                objwsRef.unMsg.method = "order.unsubscrib";
                objwsRef.ws.send(JSON.stringify(objwsRef.unMsg));
            }catch (err){
                logger.error(`coinex future privateSocket no connection to close.err: ${err.message}`)
            }
