...
class PrivateSocekt{
    ...
    async getToken(){
        ...
        try{
            ...
            await this.axiosInstance.post(endpoint, '', {
                ...
                let dicData = {
                    code:400,
                    msg: 'kucoin.socket.private.getToken:unknown error'
                }
                if(err.response){
                ...
                logger.error(`kucoin.spot.socket.private.getToken.err: ${err}`);
            })
            logger.info(`kucoin.spot.socket.public.getToken.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`kucoin.spot.socket.private.getToken.error: ${error}`);
            return response;
        }
    }
        ...
        const initConnect = () => {
            ...
            ws.on('close', (closeEventCode, reason) => {
                if (!wsRef.closeInitiated) {
                    logger.error(`kucoin spot privateSocket connection close due to ${closeEventCode}: ${reason}.`)
                    setTimeout(() => {
                        logger.debug('kucoin spot privateSocket reconnect to the server.')
                        initConnect()
                    }, this.reconnectDelay)
                } else {
                    wsRef.closeInitiated = false
    ...
    unsubscribe (objRef) {
        if (!objRef || !objRef.ws) logger.warn('kucoin spot privateSocket no connection to close.')
        else {
            try {
                objRef.unMsg.type = "unsubscribe";
                objRef.ws.send(JSON.stringify(objRef.unMsg));
            }catch (err){
                logger.error(`kucoin spot privateSocket no connection to close.err: ${err.message}`)
            }
