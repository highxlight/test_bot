...
class PublicSocket{
    ...
    async getToken(){
        ...
        try{
            ...
            logger.debug(`kucoin.future.socket.public.getToken.url: ${endpoint}`)
            await this.axiosInstance.post(endpoint).then(res=>{
                ...
                let dicData = {
                    code:400,
                    msg: 'kucoin.future.socket.public.getToken:unknown error'
                }
                if(err.response){
                ...
                logger.error(`kucoin.future.socket.public.getToken.err: ${err}`);
            })
            logger.info(`kucoin.future.socket.public.getToken.response: ${JSON.stringify(response)}`)
            return response;
        } catch (error) {
            logger.error(`kucoin.future.socket.public.getToken.error: ${error}`);
            return response;
        }
    }
        ...
        switch (e_type){
            case "level2":
                try {
                    ...
                    this.symbolOrderBookEE.emit('KUCOIN_FUTURE_OB_UPDATES', orderBookData);
                }catch (err){
                    logger.error(`kucoin.future.socket.publicSocket.orderbook.err:${err.message}, pl: ${JSON.stringify(pl)}`);
                }
                break;
        }
    }
        ...
        const initConnect = () => {
            ...
            ws.on('open', ()=>{
                ...
                wsRef.timer = setInterval( ()=>{
                    try {
                        ...
                        logger.error(`kucoin.future.public.ping.err: ${err.message}`)
                    }
                }, pingInterval)
            ...
            ws.on('close', (closeEventCode, reason) => {
                if (!wsRef.closeInitiated) {
                    logger.error(`kucoin future publicSocket connection close due to ${closeEventCode}: ${reason}.`)
                    setTimeout(() => {
                        logger.debug('kucoin future publicSocket reconnect to the server.')
                        initConnect()
                    }, this.reconnectDelay)
                } else {
                    wsRef.closeInitiated = false
    ...
    unsubscribe (objRef) {
        if (!objRef || !objRef.ws) logger.warn('kucoin future publicSocket no connection to close.')
        else {
            try {
                ...
                logger.error(`kucoin future publicSocket no connection to close.err: ${err.message}`)
            }
